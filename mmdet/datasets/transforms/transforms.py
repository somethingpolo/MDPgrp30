...
@TRANSFORMS.register_module()
class RandomFlipExcludeClass(MMCV_RandomFlip):
    ...
    """
    def __init__(self, exclude_class_ids=None, **kwargs):
    ...
    def transform(self, results):
        # Check if any of the specified classes are in the image
        if 'gt_bboxes_labels' in results and any(label in self.exclude_class_ids for label in results['gt_bboxes_labels']):
            # If any specified class is present, skip the flip
            results['flip'] = False
            results['flip_direction'] = None
        else:
            # If none of the specified classes are present, proceed with the flip
